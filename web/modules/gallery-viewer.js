...
async function fetchGalleryImages(char) {
    ...
    return mediaFiles.map(fileName => {
        ...
        return {
            name: fileName,
            // ARCHIVE FORK (see web/VENDORED.md): this url becomes an <img>/<video>
            // src, which never passes through fetch(), so it points at the archive
            // directly rather than relying on archive-api.js to rewrite it.
            url: `/api/v1/galleries/${encodeURIComponent(safeFolderName)}/files/${encodeURIComponent(fileName)}`,
