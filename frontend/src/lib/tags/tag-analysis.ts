// tag-analysis.ts
// Pure tag logic — no DOM, no SillyTavern globals. Ported verbatim (behaviour
// unchanged) from web/vendor/tag-tools/tag-analysis.js as salvage item 2 of the
// UI rewrite (docs/UI_REWRITE_PLAN.md §2); its test suite comes across with it
// as the acceptance gate (tag-analysis.test.ts).
//
// The dictionary is mapping-driven: a persistent `{ canonical: [variant…] }`
...
// is flattened to { canonical: [alias…] } on load (see dictionary.ts).
//
...
// POST /api/v1/tags/apply's job.

/** A card as this module reads it: only the tags and an avatar key are used. */
...
/** A variant chip: an observed or declared tag inside a bucket. */
...
/** A canonical group: its display variants and its inert glob rules. */
...
/**
 * Read the embedded card tags off a character object.
 * Prefers data.tags (the real V2/V3 field) and falls back to the root mirror.
 */
export function getCardTags(char: CardLike): string[] {
 ...
 * "Female" twice only counts once. Keyed by the exact tag string.
 */
function scanTags(
  characters: CardLike[],
): Map<string, { count: number; avatars: Set<string> }> {
  const stats = new Map<string, { count: number; avatars: Set<string> }>()
  for (const char of characters ?? []) {
    const avatar = char?.avatar ?? ''
    const seen = new Set<string>() // lowercase tags already counted for this card
    for (const tag of getCardTags(char)) {
      ...
      if (!entry) {
        entry = { count: 0, avatars: new Set<string>() }
        stats.set(tag, entry)
      }
      entry.count++
 ...
 */
export function parsePattern(entry: string): Pattern | null {
...
export const isPattern = (entry: string): boolean =>
  parsePattern(entry) !== null

/** Does a parsed pattern match an already-normalized tag key? */
function patternMatches(p: Pattern, key: string): boolean {
...
const patternRank = (p: Pattern): number => (p.kind === 'contains' ? 0 : 1)
 ...
 */
export function sortPatterns<P extends Pattern>(patterns: P[]): P[] {
 ...
 * @param sorted  output of {@link sortPatterns}
 * @param tag  raw tag; normalized here
 */
export function matchPattern<P extends Pattern>(
  sorted: P[],
  tag: string,
): P | undefined {
  const key = norm(tag)
 ...
 */
export function splitEntries(entries: string[]): {
  aliases: string[]
  patterns: string[]
} {
  ...
  for (const e of entries ?? [])
    (isPattern(e) ? patterns : aliases).push(String(e))
...
export function buildBuckets(
  ...
  const mapPatterns: Array<Pattern & { canonical: string }> = []
  ...
  const removedMap = new Map<string, Variant>()
  const removedLookup = new Set<string>()
  const removePatterns: Pattern[] = []
  for (const t of removedTags ?? []) {
    const p = parsePattern(t)
    if (p) {
      removePatterns.push(p)
      continue
    }
    removedLookup.add(norm(t))
    if (!removedMap.has(String(t)))
      removedMap.set(String(t), {
        tag: String(t),
        count: 0,
        avatars: [],
        declared: true,
  ...
  const groupMap = new Map<string, Map<string, Variant>>()
  const ensure = (c: string): Map<string, Variant> => {
    let g = groupMap.get(c)
    if (!g) {
      g = new Map<string, Variant>()
      groupMap.set(c, g)
    }
    return g
  }
  ...
  for (const [tag, entry] of stats) {
    ...
    if (canonical) {
      ...
      g.set(tag, {
        ...variant,
        declared,
        matchedBy: declared ? 'declared' : 'norm',
      })
      continue
    }
    ...
    if (hit) {
      ensure(hit.canonical).set(tag, {
        ...variant,
        declared: false,
        matchedBy: `pattern:${hit.source}`,
      })
      continue
    }
  ...
  const byCount = (a: Variant, b: Variant) =>
    b.count - a.count || a.tag.localeCompare(b.tag)
  const groups: Group[] = [...groupMap.entries()].map(([canonical, vmap]) => ({
    canonical,
 ...
 *
 * This is the whole editor/server contract. Every match decision — norm(),
...
export function buildApplyPayload(
  ...
  return {
    rename,
    remove: removed.filter((v) => v.count > 0).map((v) => v.tag),
  }
}
