...
// SERVER/log/warn) and keep readyState "loading" so boot() itself is
// deferred to a DOMContentLoaded listener that's never fired. window/XHR
// just need to exist for FetchHook.install() to patch them without throwing.
function load() {
  function XHRStub() {}
  return loadModules(
